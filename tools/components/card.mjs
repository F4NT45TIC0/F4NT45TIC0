...
const TAG_COLOR = { LIVE: C.signal, PRIVATE: C.pink, REPO: C.cyan, STARRED: C.gold };
...
export function card(c, index) {
  ...
  const border = c.starred ? C.gold : 'url(#holo)';
  ...
  const title = textPath(c.title, { font: 'display', size: c.titleSize ?? 30, x: PAD, y: 104, tracking: 2 });
  ...
  const body = `<path d="${chamfer(1, 1, CARD_W - 2, CARD_H - 2, 16)}" fill="${C.panel}" stroke="${border}" stroke-width="2"/>
...
<g fill="none" stroke="${accent}" stroke-opacity=".6">${frames}</g>
<g fill="${C.dim}">${labels}</g>
<path class="tw" d="${star(CARD_W - 40, 93, 9)}" fill="${c.starred ? C.gold : 'url(#holo)'}"/>`;
